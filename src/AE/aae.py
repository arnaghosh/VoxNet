...
#    D_loss = -torch.mean(torch.log(D_real_gauss + EPS) + torch.log(1 - D_fake_gauss + EPS))
    D_loss = F.nll_loss(D_out,(labels-1).type(torch.cuda.LongTensor))
...
#    G_loss.backward()
    optim_Q_gen.step()   
    acc = accuracy_metric(D_out,labels)
#    '''
...
#         (3) Log the images
        info = {
            'images': to_np(images.view(-1, 1, 84)[:10]),
            'recons_images': to_np(X_sample.view(-1, 1, 84)[:10])
        }
